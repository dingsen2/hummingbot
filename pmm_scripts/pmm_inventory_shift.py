...
class PMMInventoryShift(ScriptStrategyBase):
    ...
    # At least one exchange/pair needs to be instantiated
    ...
    def update_multipliers(self):
        ...
        base_bal_in_quote = base_bal * self.connectors[self.exchange].get_price_by_type(self.trading_pair,
                                                                                        self.price_source)
        ...
        self.reference_price = self.orig_price * Decimal(str(1 + self.price_multiplier)) * Decimal(
            str(1 + self.inventory_multiplier))
    ...
    def format_status(self) -> str:
        ...
        lines.extend([f"  Trend Scalar: {self.trend_scalar:.1f} |"
                      f" Trend Multiplier (bps): {self.price_multiplier * 10000:.4f} |"
                      f" Trend Price Shift: {trend_price_shift:.4f}"])
        lines.extend([f"  Target Inventory Ratio: {self.target_ratio:.4f} |"
                      f" Current Inventory Ratio: {self.current_ratio:.4f} |"
                      f" Inventory Delta: {self.inventory_delta:.4f}"])
        lines.extend([f"  Inventory Multiplier (bps): {self.inventory_multiplier * 10000:.4f} |"
                      f" Inventory Price Shift: {inventory_price_shift:.4f}"])
        lines.extend([f"  Orig Price: {self.orig_price:.4f} |"
                      f" Reference Price: {self.reference_price:.4f}"])
        lines.extend(["\n----------------------------------------------------------------------\n"])
        candles_df = self.get_candles_with_features()
